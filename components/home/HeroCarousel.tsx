...
export function HeroCarousel() {
  ...
  return (
    <section className="relative min-h-[calc(100svh-5rem)] w-full overflow-hidden">
      ...
      <div className="relative z-10 flex min-h-[calc(100svh-5rem)] items-end px-4 pb-10 pt-12 sm:px-8 sm:pb-14 lg:items-center lg:px-14 xl:px-20 2xl:px-24">
        <div className="max-w-3xl space-y-6">
          <span className="inline-flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.2em] text-[#e1c19a]">
            <span className="h-px w-8 bg-[#e1c19a]" />
            Institutional Advisory
          </span>

          <h1 className="text-4xl font-extrabold leading-tight text-[#dee3ea] sm:text-5xl lg:text-6xl">
            {slides[activeIndex].title}
          </h1>

          {slides[activeIndex].isNew ? (
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[#a88c68] bg-[#a88c68]/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-[#e1c19a]">
              <span className="h-2 w-2 rounded-full bg-[#e1c19a]" />
              {slides[activeIndex].newLabel}
            </div>
          ) : null}

          <p className="max-w-2xl text-base leading-8 text-[#d1c4b8] sm:text-lg">
            {slides[activeIndex].description}
          </p>

          <a
            href={slides[activeIndex].ctaHref}
            className="inline-flex rounded-md bg-[#a88c68] px-6 py-3 text-xs font-semibold uppercase tracking-[0.1em] text-[#39260a] transition hover:bg-[#e1c19a]"
          >
      ...
      <div className="absolute bottom-6 right-4 z-20 flex items-center gap-2 sm:bottom-8 sm:right-8 lg:right-14 xl:right-20 2xl:right-24">
        <button
          type="button"
          onClick={goPrev}
          aria-label="Previous slide"
          className="rounded-md border border-[#4e453c] bg-[#171c21]/80 px-3 py-2 text-[#dee3ea] transition hover:border-[#e1c19a] hover:text-[#e1c19a]"
        >
          ←
        </button>
        <button
      ...
      <div className="absolute bottom-6 left-4 z-20 flex items-center gap-2 sm:bottom-8 sm:left-8 lg:left-14 xl:left-20 2xl:left-24">
        {slides.map((slide, index) => (
          <button
            key={slide.title}
            type="button"
